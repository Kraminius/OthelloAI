...
public class OthelloController {
    ...
    public void updateAIProgress(int progress, int end, boolean player) {
        ...
        if (Config.getTurn() && (Config.getGameType().equals(GameType.PLAYER_VS_AI))) {right.setAIProgressLabel(text);  if(progress > (0.9*end)) right.setAIProgressLabel("Waiting...");}
        else if (!player && (Config.getGameType().equals(GameType.AI_VS_AI))) {left.setAIProgressLabel(text); if(progress >= (0.9*end)) {left.setAIProgressLabel("Waiting...");}}
        else if (player && (Config.getGameType().equals(GameType.AI_VS_AI))) {right.setAIProgressLabel(text);  if(progress >= (0.9*end)) right.setAIProgressLabel("Waiting...");}
    }
